...
namespace Drupal\organization_feed;
...
class Helper {
  ...
  private function createHttpRequest($endpoint) {
    ...
    try {
      ...
      \Drupal::logger('organization_feed')->notice($e);
    }
  }
  /**
  ...
  public static function getOrganizationsFromFeed() {
    $endpoint = \Drupal::config('organization_feed.endpoints')->get('organization');
  ...
  public static function getExistingOrganizations() {
    $organizations = \Drupal::database()
                  ->query("SELECT nid
                    ...
                    WHERE type='organization'")->fetchCol();
